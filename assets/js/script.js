...
document.querySelector('#addFileCab').addEventListener('click', addFileCabToArray);
...
function addFileCabToArray(){
  ...
  if(isTaken){
    ...
    if(fileCabArray.length === 10){
      ...
      ui.showAlert('You have reached the maximun number of File Cabinets', 'error');
      return;
    }
...
primaryArray[primaryObjIndex].secondaryArray[secondaryObjIndex].noteArray.push(newNote);
...
primaryArray[primaryObjIndex].secondaryArray.forEach((element, index) =>{
 ...
 primaryArray[primaryObjIndex].secondaryArray[secondaryObjIndex].noteArray[currentNoteIndex].imagePath = fullImagePath;
//set primary array back to file
myStorage.setArrayToFileName(primaryArray, fileName);
addImageAudio.play();
...
addAudio.play();
ui.showAndCheckTextArea();
ui.showAlert('Copy and paste the text in the text area to a .json file and add it to the json folder', 'success', 10000)
...
if(textAreaInput === ""){
  warningEmptyAudio.play();
  ui.showAlert('Please copy and paste a valid JSON string in the large text Area', 'error', 10000);
  ui.showAndCheckTextArea();
  return;
}
...
if(checkArray.length === 0){
  ...
  if(confirm('Are you sure, you want to load this data, this file cabinet already has valid data?')){
    myStorage.setArrayToFileName(array, fileName);
    addAudio.play();
    ui.showAlert('File Cabinet loaded!', 'success');
  }else{
...
currentNoteIndex = deleteIndex;
  ...
  let selectedNote = primaryArray[primaryObjIndex].secondaryArray[secondaryObjIndex].noteArray[currentNoteIndex];
...
primaryArray[primaryObjIndex].secondaryArray[secondaryObjIndex].noteArray.splice(deleteIndex, 1);
//set primary array back to file
myStorage.setArrayToFileName(primaryArray, fileName);
